...
from utils.metrics import box_iou, fitness, diou_box_nms
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # i = diou_box_nms.nms(boxes, scores, iou_thres)  # NMS
        ...
        output[xi] = x[i]
        dets = x[i]
        boxes_list.append(dets[:, 0:4] / imgsz[0])
        scores_list.append(dets[:, 4])
        labels_list.append(dets[:, 5])
    ...
    boxes, scores, labels = weighted_boxes_fusion(boxes_list, scores_list, labels_list,
                                                  weights=np.ones(len(boxes_list)), iou_thr=iou_thres,
                                                  skip_box_thr=0.0001)  # NMS  # thr 可以调节 去掉多重

    boxes, scores, labels = box_filter( boxes, scores, labels , imgsz)
    for j, score in enumerate(scores):
        output.append(
            [boxes[j][0] * imgsz[0], boxes[j][1] * imgsz[0], boxes[j][2] * imgsz[0], boxes[j][3] * imgsz[0],
             scores[j], labels[j]])
