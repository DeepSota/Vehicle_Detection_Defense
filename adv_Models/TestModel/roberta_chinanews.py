...
from adv_Models.hf_model import HuggingFaceNLPVictimModel
...
class VictimRoBERTaChinaNews(HuggingFaceNLPVictimModel):
    """
    7分类模型，中文
    """

    __name__ = "VictimRoBERTaChinaNews"

    def __init__(self, path: Optional[str] = None) -> NoReturn:
        """ """
        self._path = path or os.path.join(
            nlp_cache_dir, "roberta-base-finetuned-chinanews-chinese"
        )
        if not os.path.exists(self._path):
            # raise ValueError("暂不支持在线下载模型")
            download_if_needed(
                uri="roberta-base-finetuned-chinanews-chinese",
                source="aitesting",
