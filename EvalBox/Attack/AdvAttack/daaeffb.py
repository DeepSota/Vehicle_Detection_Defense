...
class DualAttentionAttack(Attack):
    ...
    def _parse_params(self, **kwargs):
        ...
        @return: None
        """
        self.patchsize = int(kwargs.get("patchsize", "20"))
    ...
    def generate(self, xs=None, ys=None):
        ...
        for idx in range(batchsize):
            ...
            # centerx = x.shape[2] // 2
            # centery = x.shape[3] // 2
            centerx = random.randint(80, 170)
            centery = random.randint(80, 170)
...
# self.model_layer = self.model._modules.get(conv_layer)
# self.hook_handles.append(self.model_layer.register_forward_hook(self._hook_a))
...
class GradCam():
    ...
    def __init__(self, model, conv_layer, device):
        ...
        #
        # self.hook_handles.append(self.model.model.model._modules.get(conv_layer).register_forward_hook(self._hook_a))
        #
        # self._relu = True
        # self._score_uesd = True
        # self.hook_handles.append(self.model.model.model._modules.get(conv_layer).register_backward_hook(self._hook_g))

        self.model = model.eval()
        self.model.to(device)
        self.fx = self.model.model.model.blocks[6]
        self.hook_handles.append(self.fx.register_forward_hook(self._hook_a))  # 只要找到对应层就行不需要 使用get layer

        self._relu = True
        self._score_uesd = True
        self.hook_handles.append(self.fx.register_backward_hook(self._hook_g))
