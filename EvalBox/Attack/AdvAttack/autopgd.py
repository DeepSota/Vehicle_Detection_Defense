...
class AutoPGD(Attack):
    def __init__(self, model=None, device=None, IsTargeted=None, **kwargs):
        ...
        @return: None
        """
        super(AutoPGD, self).__init__(model, device, IsTargeted)
    ...
    def _parse_params(self, **kwargs):
        self.n_iter = int(kwargs.get("n_iter", 100))
        self.eps = float(kwargs.get("eps", 0.06))
        self.norm = kwargs.get("norm", 'Linf')
        self.n_restarts = int(kwargs.get("n_start", 1))
        self.seed = int(kwargs.get("seed", 0))
        self.loss = kwargs.get("loss", 'dlr')
        self.eot_iter = int(kwargs.get("eot_iter", 1))
        self.thr_decr = float(kwargs.get("thr_decr", 0.75))
        self.verbose = False
    ...
    def attack_single_run(self, x_in, y_in,compute_loss):
        ...
        if self.norm == 'Linf':
            t = 2 * torch.rand(x.shape) - 1
            x_adv = x + self.eps * torch.ones([x.shape[0], 1, 1, 1]) * t / (t.reshape([t.shape[0], -1]).abs().max(dim=1, keepdim=True)[0].reshape([-1, 1, 1, 1]))
        
        elif self.norm == 'L2':
            t = torch.randn(x.shape)
            x_adv = x + self.eps * torch.ones([x.shape[0], 1, 1, 1]) * t / ((t ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12)
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                logits = self.model(x_adv.to(self.device)).to(x_adv.device) # 1 forward pass (eot_iter = 1)
                loss_indiv = compute_loss(logits[1], y)# criterion_indiv(logits, y)
                loss = loss_indiv.sum()
        ...
        step_size = self.eps * torch.ones([x.shape[0], 1, 1, 1]) * torch.Tensor([2.0]).reshape([1, 1, 1, 1])
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12) * torch.min(
                        self.eps * torch.ones(x.shape), ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt()), 0.0, 1.0)
                    x_adv_1 = x_adv + (x_adv_1 - x_adv) * a + grad2 * (1 - a)
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12) * torch.min(
                        self.eps * torch.ones(x.shape), ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12), 0.0, 1.0)
            ...
            for _ in range(self.eot_iter):
                with torch.enable_grad():
                    logits = self.model(x_adv.to(self.device)).to(x_adv.device) # 1 forward pass (eot_iter = 1)
                    loss_indiv, loss_items = compute_loss(logits[1], y)
                    loss = loss_indiv.sum()
    ...
    def generate(self, x_in, y_in, compute_loss=None):
        assert self.norm in ['Linf', 'L2']
        x = x_in.clone() if len(x_in.shape) == 4 else x_in.clone().unsqueeze(0)
        y = y_in.clone() if len(y_in.shape) == 1 else y_in.clone().unsqueeze(0)

        adv = x.clone()
        acc = (self.model(x.to(self.device)).max(1)[1] == y.to(self.device)).to(x_in.device)
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                best_curr, acc_curr, loss_curr, adv_curr = self.attack_single_run(x_to_fool, y_to_fool, compute_loss)
